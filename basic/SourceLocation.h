...
namespace llvm {
  ...
  /// DenseSets.
  template <>
  struct DenseMapInfo<ellcc::FileID> {
    static inline ellcc::FileID getEmptyKey() {
      return ellcc::FileID();
    }
    static inline ellcc::FileID getTombstoneKey() {
      return ellcc::FileID::getSentinel(); 
    }
    
    static unsigned getHashValue(ellcc::FileID S) {
      return S.getHashValue();
    }
    
    static bool isEqual(ellcc::FileID LHS, ellcc::FileID RHS) {
      return LHS == RHS;
    }
