...
gld${EMULATION_NAME}_map_segments (bfd_boolean need_layout)
{
  ...
  do
	  ...
	  && !link_info.relocatable)
	{
	  ...
	  if (!_bfd_elf_map_sections_to_segments (link_info.output_bfd, &link_info))
	    einfo ("%F%P: map sections to segments failed: %E\n");

	  if (phdr_size != elf_tdata (link_info.output_bfd)->program_header_size)
	    {
	      if (tries > 6)
		/* The first few times we allow any change to
		   phdr_size .  */
		need_layout = TRUE;
	      else if (phdr_size < elf_tdata (link_info.output_bfd)->program_header_size)
		/* After that we only allow the size to grow.  */
		need_layout = TRUE;
	      else
		elf_tdata (link_info.output_bfd)->program_header_size = phdr_size;
	    }
	}
    }
