...
static int nios2_disassemble (bfd_vma, unsigned int, disassemble_info *);
static void nios2_init_opcode_hash (void);


static int nios2_print_insn_arg (const char *argptr, unsigned int opcode,
  ...
  if (status == 0)
    {
      ...
      if (endianness == BFD_ENDIAN_BIG)
        insn = (unsigned int) bfd_getb32 (buffer);
      else
        insn = (unsigned int) bfd_getl32 (buffer);
...
nios2_find_opcode_hash (unsigned int opcode)
{
...
int
nios2_disassemble (bfd_vma address, unsigned int opcode,
...
nios2_print_insn_arg (const char *argptr,
		      unsigned int opcode, bfd_vma address,
		      disassemble_info * info)
{
  unsigned int i = 0;
