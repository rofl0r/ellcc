  ...
  *again = FALSE;
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
      ...
      if (ELF32_R_TYPE (irel->r_info) == (int) R_NIOS2_UJMP)
	{
	  ...
	  Elf_Internal_Rela *local_irelend = irel + sec->reloc_count;

	  for (; irela < local_irelend; irela++)
	    {
	  ...
	  if ((irelalign == NULL && (int) pcrel_offset <= 0x8004
	       && (int) pcrel_offset >= -0x8000) || (irelalign != NULL
						      && (int) pcrel_offset
						      <= 0x7ffc
						      && (int) pcrel_offset
						      >= -0x8000))
      ...
      if (ELF32_R_TYPE (irel->r_info) == (int) R_NIOS2_CJMP)
	{
	  ...
	  /* does this value fit in 16 bits */
	  if ((irelalign == NULL && (int) pcrel_offset <= 0x8008
	       && (int) pcrel_offset >= -0x8000) || (irelalign != NULL
						      && (int) pcrel_offset
						      <= 0x7ffc
						      && (int) pcrel_offset
  ...
  for (rel = relocs; rel < relend; rel++)
      ...
      if (howto != NULL)
	{
	  switch (howto->type)
		...
		local_off = htab->tls_ldm_got.offset;
		...
		else
		  {
			...
			bfd_elf32_swap_reloca_out (output_bfd, &outrel, loc);
		      ...
		      bfd_put_32 (output_bfd, 1,
				  htab->sgot->contents + local_off);

		    htab->tls_ldm_got.offset |= 1;
		  }

		relocation = (htab->sgot->output_offset + local_off);
		...
		if (h != NULL)
		...
		else
		  {
		    if (local_got_offsets == NULL)
		      abort ();
		    local_off = local_got_offsets[r_symndx];
