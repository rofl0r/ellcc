...
private:
    class _SourceManager {
      ...
      /// _SourceManager.  This set allows us to merge ContentCache entries based
      ...
      // _SourceManager doesn't support copy construction.
      explicit _SourceManager(const _SourceManager&);
      void operator=(const _SourceManager&);  
    public:
      _SourceManager() 
      ...
      sm::string toString(SourceLocation loc) const
      {
        ellcc::PresumedLoc ploc = getPresumedLoc(loc);
      ...
      /// hot method that is used for all _SourceManager queries that start with a
    ...
    static _SourceManager* SM;
    /** The number of instances created.
     */
    static unsigned instances;

    SourceManager(const SourceManager&);        // Never used.
public:
    SourceManager()
        { if (SM == NULL) SM = new _SourceManager(); ++instances; }
    ~SourceManager()
        { if (SM && --instances == 0) { delete SM; SM = NULL; } }
  void clearIDTables() { SM->clearIDTables(); }
  sm::string toString(SourceLocation loc) const { return SM->toString(loc); }
  FileID getMainFileID() const { return SM->getMainFileID(); }
  FileID createMainFileID(const FileEntry *SourceFile, SourceLocation IncludePos)
    { return SM->createMainFileID(SourceFile, IncludePos); }
  FileID createFileID(const FileEntry *SourceFile, SourceLocation IncludePos,
                      SrcMgr::CharacteristicKind FileCharacter = SrcMgr::C_User,
                      unsigned PreallocatedID = 0,
                      unsigned Offset = 0)
    { return SM->createFileID(SourceFile, IncludePos, FileCharacter, PreallocatedID, Offset); }
  FileID createFileIDForMemBuffer(const llvm::MemoryBuffer *Buffer,
                                  unsigned PreallocatedID = 0,
                                  unsigned Offset = 0)
    { return SM->createFileIDForMemBuffer(Buffer, PreallocatedID, Offset); }
  FileID createMainFileIDForMemBuffer(const llvm::MemoryBuffer *Buffer)
    { return SM->createMainFileIDForMemBuffer(Buffer); }
  SourceLocation createInstantiationLoc(SourceLocation Loc,
                                        SourceLocation InstantiationLocStart,
                                        SourceLocation InstantiationLocEnd,
                                        unsigned TokLength,
                                        unsigned PreallocatedID = 0,
                                        unsigned Offset = 0)
    { return SM->createInstantiationLoc(Loc, InstantiationLocStart, InstantiationLocEnd,
                                        TokLength, PreallocatedID, Offset); }
  const llvm::MemoryBuffer *getBuffer(FileID FID) const
  ...
  FileID getFileID(SourceLocation SpellingLoc) const
    { return SM->getFileID(SpellingLoc); }
  SourceLocation getLocForStartOfFile(FileID FID) const
    { return SM->getLocForStartOfFile(FID); }
  SourceLocation getInstantiationLoc(SourceLocation Loc) const
    { return SM->getInstantiationLoc(Loc); }
  std::pair<SourceLocation,SourceLocation> getImmediateInstantiationRange(SourceLocation Loc) const
    { return SM->getImmediateInstantiationRange(Loc); }
  std::pair<SourceLocation,SourceLocation> getInstantiationRange(SourceLocation Loc) const
    { return SM->getInstantiationRange(Loc); }
  SourceLocation getSpellingLoc(SourceLocation Loc) const
  ...
  std::pair<FileID, unsigned> getDecomposedInstantiationLoc(SourceLocation Loc) const
    { return SM->getDecomposedInstantiationLoc(Loc); }
  std::pair<FileID, unsigned> getDecomposedSpellingLoc(SourceLocation Loc) const
    { return SM->getDecomposedSpellingLoc(Loc); }
  unsigned getFileOffset(SourceLocation SpellingLoc) const
    { return SM->getFileOffset(SpellingLoc); }
  const char *getCharacterData(SourceLocation SL) const
    { return SM->getCharacterData(SL); }
  unsigned getColumnNumber(FileID FID, unsigned FilePos) const
    { return SM->getColumnNumber(FID, FilePos); } 
  unsigned getSpellingColumnNumber(SourceLocation Loc) const
    { return SM->getSpellingColumnNumber(Loc); }
  unsigned getInstantiationColumnNumber(SourceLocation Loc) const
    { return SM->getInstantiationColumnNumber(Loc); }
  unsigned getLineNumber(FileID FID, unsigned FilePos) const
    { return SM->getLineNumber(FID, FilePos); }
  unsigned getInstantiationLineNumber(SourceLocation Loc) const
    { return SM->getInstantiationLineNumber(Loc); }
  unsigned getSpellingLineNumber(SourceLocation Loc) const
    { return SM->getSpellingLineNumber(Loc); }
  const char *getBufferName(SourceLocation Loc) const
    { return SM->getBufferName(Loc); }
  SrcMgr::CharacteristicKind getFileCharacteristic(SourceLocation Loc) const
    { return SM->getFileCharacteristic(Loc); }
  PresumedLoc getPresumedLoc(SourceLocation Loc) const
    { return SM->getPresumedLoc(Loc); }
  bool isFromSameFile(SourceLocation Loc1, SourceLocation Loc2) const
    { return SM->isFromSameFile(Loc1, Loc2); }
  bool isFromMainFile(SourceLocation Loc) const
    { return SM->isFromMainFile(Loc); }
  bool isInSystemHeader(SourceLocation Loc) const
    { return SM->isInSystemHeader(Loc); }
  unsigned getLineTableFilenameID(const char *Ptr, unsigned Len)
    { return SM->getLineTableFilenameID(Ptr, Len); }
  void AddLineNote(SourceLocation Loc, unsigned LineNo, int FilenameID)
    { SM->AddLineNote(Loc, LineNo, FilenameID); }
  void AddLineNote(SourceLocation Loc, unsigned LineNo, int FilenameID,
                   bool IsFileEntry, bool IsFileExit, 
                   bool IsSystemHeader, bool IsExternCHeader)
    { SM->AddLineNote(Loc, LineNo, FilenameID, IsFileEntry, IsFileExit, IsSystemHeader, IsExternCHeader); }
  bool hasLineTable() const { return SM->hasLineTable(); }
  LineTableInfo &getLineTable() { return SM->getLineTable(); }
  fileinfo_iterator fileinfo_begin() const { return SM->fileinfo_begin(); }
  fileinfo_iterator fileinfo_end() const { return SM->fileinfo_end(); }
  void PrintStats() const { SM->PrintStats(); }
  sloc_entry_iterator sloc_entry_begin() const { return SM->sloc_entry_begin(); }
  sloc_entry_iterator sloc_entry_end() const { return SM->sloc_entry_end(); }
  unsigned sloc_entry_size() const { return SM->sloc_entry_size(); }
  const SrcMgr::SLocEntry &getSLocEntry(FileID FID) const
    { return SM->getSLocEntry(FID); }
  unsigned getNextOffset() const { return SM->getNextOffset(); }
  void PreallocateSLocEntries(ExternalSLocEntrySource *Source,
                              unsigned NumSLocEntries,
                              unsigned NextOffset)
    { return SM->PreallocateSLocEntries(Source, NumSLocEntries, NextOffset); }
  void ClearPreallocatedSLocEntries() { SM->ClearPreallocatedSLocEntries(); }
